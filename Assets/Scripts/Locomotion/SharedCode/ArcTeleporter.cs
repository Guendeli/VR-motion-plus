...
public class ArcTeleporter : MonoBehaviour {
    ...
    public enum MoveType { Teleport, Dash}
		...
		if (lastTriggerState && !currentTriggerState) {
			...
			if (arcRaycaster!= null && arcRaycaster.MakingContact) {
				if (objectToMove != null) {
                    ...
                    {
                        // DASH
                        StartCoroutine(DoDash(arcRaycaster.HitPoint + up * height));
                    }
				}
			}
    ...
    private IEnumerator DoDash(Vector3 endPoint)
    {
        ...
        Vector3 startPoint = objectToMove.position;
        ...
        while (elapsed < dashTime)
        {
            ...
            objectToMove.position = Vector3.Lerp(startPoint, endPoint, elapsedPct);
            yield return null;
        }
